...
func New(certPath, keyPath string, logger *slog.Logger) (*CertWatcher, error) {
	var err error

	cw := &CertWatcher{
		// NB: We set the logger here, as it is used in ReadCertificate
		logger:   logger.With("certPath", certPath, "keyPath", keyPath),
		certPath: certPath,
		keyPath:  keyPath,
	}
...
func (cw *CertWatcher) Watch() {
	for {
		...
		case err, ok := <-cw.watcher.Errors:
			...
			cw.logger.Error("certificate watch error", slog.Any("error", err))
		}
	}
}
...
func (cw *CertWatcher) handleEvent(event fsnotify.Event) {
	...
	cw.logger.Debug("certificate event", "event", event)

	// If the file was removed, re-add the watch.
	if isRemove(event) {
		if err := cw.watcher.Add(event.Name); err != nil {
			cw.logger.Error("error re-watching file", slog.Any("error", err))
		}
	}

	if err := cw.ReadCertificate(); err != nil {
		cw.logger.Error("error re-reading certificate", slog.Any("error", err))
	}
}
